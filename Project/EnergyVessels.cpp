...
void EnergyVessels::SetBarStatu() {
	...
	for (int i = 0;i < _maxEnergyValue;i++) {
		...
		_energyBarArray[i].SetBasePosition(Vector2(_vesselsPosition.x + _energyVesselsTexture.GetWidth() * _scale / 2, _vesselsPosition.y + _energyVesselsTexture.GetHeight() * _scale / 2));
